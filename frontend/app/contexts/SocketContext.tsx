...
const REALTIME_GATEWAY_URL =
    import.meta.env.VITE_REALTIME_GATEWAY_URL ?? "ws://localhost:3000";
...
type SocketContextProps = {
    isConnected: boolean;
    onMessage: (
        callback: (data: NewMessageResponse) => void,
    ) => () => void;
};
...
export const SocketProvider = ({ children }: { children: ReactNode }) => {
    ...
    const messageListeners =
        useRef<Set<(data: NewMessageResponse) => void>>(new Set());

    useEffect(() => {
        if (!isAuthenticated || !user) return;

        const socketInstance = io(REALTIME_GATEWAY_URL, {
            auth: {
                userId: user.id,
            },
        });
